...
import com.sharifin.retrofit.R
...
fun getStatusCodeErrorHolder(code: Int, errorMessage: String): ErrorHolder =
        if (code == STATUS_UNAUTHORIZED) {
            getUnAuthorizedMessage(errorMessage)
        } else {
            ErrorHolder.StringRes(getStatusCodeErrorString(code))
        }

fun getUnAuthorizedMessage(errorMessage: String): ErrorHolder =
        if (errorMessage.isNotEmpty()) {
            ErrorHolder.UnAuthorized(errorMessage)
        } else {
            ErrorHolder.UnAuthorized("مدت زمان کاری شما منقضی شده است")
        }
...
fun <T> Response<T>.getErrorHolder(): ErrorHolder {
    ...
    return if (errorMessage.isEmpty() || code() == STATUS_UNAUTHORIZED) {
        ...
        ErrorHolder.Message(errorMessage)
    }
}

fun <T> Response<T>.getErrorHolderForLogin(): ErrorHolder {
    val errorMessage = errorBody()?.use { it.string() }?.genericErrorParseFirstError() ?: ""
...
fun String.genericErrorParseFirstError(): String {
    try {
        ...
        Timber.e("error_parser", "failed to parse generic response", e)
        return ""
    }
